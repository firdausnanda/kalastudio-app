...
export default function JobIndex({ auth, jobs }) {
    ...
    const handleDelete = (job) => {
        Swal.fire({
            ...
            text: `Lowongan "${job.title}" akan dihapus permanen.`,
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                router.delete(route('admin.career-jobs.destroy', job.id), {
                    preserveScroll: true,
                    onSuccess: () => Swal.fire('Terhapus!', 'Lowongan telah berhasil dihapus.', 'success')
                });
            }
        });
    };
                ...
                <main className="flex-grow p-4 md:p-8 lg:p-10 transition-all duration-300">
                    <div className="max-w-6xl mx-auto space-y-8">
                        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                            <div>
                                <h2 className="text-4xl font-black text-slate-900 dark:text-white tracking-tight">Lowongan Kerja</h2>
                                <p className="text-slate-500 dark:text-slate-400 mt-2 font-medium">Kelola informasi lowongan pekerjaan di Kalastudio.</p>
                            </div>
                            <Link
                                href={route('admin.career-jobs.create')}
                                ...
                                <table className="w-full text-left">
                                    <thead>
                                        <tr className="bg-slate-50/50 dark:bg-slate-800/20">
                                            <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Posisi & Departemen</th>
                                            <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Lokasi & Tipe</th>
                                            ...
                                            <tr>
                                                <td colSpan="4" className="px-8 py-20 text-center">
                                                    <div className="flex flex-col items-center gap-3">
                                                        <span className="material-symbols-outlined text-4xl text-slate-200">work</span>
                                                        <p className="text-slate-400 font-bold text-sm">Tidak ada lowongan ditemukan.</p>
                                                    </div>
                                                </td>
                                            </tr>
                                        ) : (
                                            filteredJobs.map((job) => (
                                                <tr key={job.id} className="hover:bg-slate-50/30 dark:hover:bg-slate-800/20">
                                                    <td className="px-8 py-6">
                                                        <p className="text-sm font-black text-slate-900 dark:text-white max-w-sm truncate">{job.title}</p>
                                                        <p className="text-xs text-slate-400 font-medium">{job.department}</p>
                                                    </td>
                                                    <td className="px-8 py-6">
                                                        <p className="text-sm font-bold text-slate-700 dark:text-slate-300">{job.location}</p>
                                                        <span className="px-2 py-0.5 bg-slate-100 dark:bg-slate-800 text-slate-500 rounded text-[9px] font-black uppercase tracking-wider">
                                                            {job.type}
                                                        </span>
                                                    </td>
                                                    <td className="px-8 py-6">
                                                        <div className="flex flex-col gap-2">
                                                            <span className={`px-3 py-1 rounded-lg text-[10px] font-black tracking-wider uppercase inline-block w-fit ${
                                                                job.is_active ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-100 text-slate-600'
                                                            }`}>
                                                                {job.is_active ? 'Aktif' : 'Nonaktif'}
                                                            </span>
                                                            {job.is_featured && (
                                                                <span className="px-3 py-1 bg-amber-100 text-amber-600 rounded-lg text-[10px] font-black tracking-wider uppercase inline-block w-fit">
                                                                    Featured
                                                                </span>
                                                            )}
                                                        </div>
