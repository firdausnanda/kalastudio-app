...
export default function TagIndex({ auth, tags }) {
    ...
    const submitCreate = (e) => {
        ...
        form.post(route('admin.blog-tags.store'), {
            ...
            onSuccess: () => {
                closeCreateModal();
                Swal.fire({
                    icon: 'success', title: 'Berhasil', text: 'Tag blog berhasil ditambahkan.',
    ...
    const submitEdit = (e) => {
        e.preventDefault();
        form.put(route('admin.blog-tags.update', selectedTag.id), {
            preserveScroll: true,
            onSuccess: () => {
                closeEditModal();
                Swal.fire({
                    icon: 'success', title: 'Berhasil', text: 'Tag blog berhasil diperbarui.',
                    timer: 2000, showConfirmButton: false
                });
            },
        });
    ...
    const handleDelete = (tag) => {
        Swal.fire({
            title: 'Hapus Tag?',
            text: `Tag "${tag.name}" akan dihapus permanen.`,
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                router.delete(route('admin.blog-tags.destroy', tag.id), {
                    preserveScroll: true,
                    onSuccess: () => Swal.fire('Terhapus!', 'Tag telah berhasil dihapus.', 'success')
                });
            }
        });
    };

    const filteredTags = tags.data.filter(t => 
        ...
        <div className="bg-slate-50 dark:bg-slate-950 min-h-screen flex flex-col font-display">
                ...
                <main className="flex-grow p-4 md:p-8 lg:p-10 transition-all duration-300">
                    <div className="max-w-6xl mx-auto space-y-8">
                        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                            <div>
                                <h2 className="text-4xl font-black text-slate-900 dark:text-white tracking-tight">Tag Blog</h2>
                                <p className="text-slate-500 dark:text-slate-400 mt-2 font-medium">Kelola tag untuk artikel blog Anda.</p>
                            </div>
                            <button
                                            ...
                                            <tr>
                                                <td colSpan="2" className="px-8 py-20 text-center">
                                                    <div className="flex flex-col items-center gap-3">
                                                        <span className="material-symbols-outlined text-4xl text-slate-200">sell</span>
                                                        <p className="text-slate-400 font-bold text-sm">Tidak ada tag ditemukan.</p>
                                                    </div>
                                                </td>
                                            </tr>
                                        ) : (
                                            filteredTags.map((tag) => (
                                                <tr key={tag.id} className="hover:bg-slate-50/30 dark:hover:bg-slate-800/20">
                                                    <td className="px-8 py-6">
                                                        <p className="text-sm font-black text-slate-900 dark:text-white">#{tag.name}</p>
                                                        <p className="text-xs text-slate-400 font-medium">{tag.slug}</p>
                                                    </td>
                                                    <td className="px-8 py-6">
            ...
            <Modal show={isCreateModalOpen} onClose={closeCreateModal} maxWidth="md">
                <form onSubmit={submitCreate} className="p-8">
                    <div className="flex items-center justify-between mb-8">
                        <div>
                            <h2 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight">Tambah Tag</h2>
                            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1 font-medium">Buat tag baru untuk mengelompokkan artikel blog Anda.</p>
                        </div>
                        <button 
                            type="button" 
                    ...
                    <div className="space-y-6">
                        <div>
                            <InputLabel value="Nama Tag" className="text-xs font-black text-slate-400 uppercase tracking-widest mb-3 ml-1" />
                            <TextInput
            ...
            <Modal show={isEditModalOpen} onClose={closeEditModal} maxWidth="md">
                <form onSubmit={submitEdit} className="p-8">
                    <div className="flex items-center justify-between mb-8">
                        <div>
                            <h2 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight">Edit Tag</h2>
                            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1 font-medium">Perbarui informasi tag "#{selectedTag?.name}".</p>
                        </div>
                        <button 
                            type="button" 
