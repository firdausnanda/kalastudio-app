...
export default function Invoice({ transaction }) {
    ...
    return (
        <div className="min-h-screen bg-slate-50 dark:bg-slate-950 font-display transition-colors duration-300 print:bg-white print:p-0">
            ...
            {/* Invoice Card */}
            <div className="max-w-4xl mx-auto my-8 md:my-12 px-4 print:my-0 print:px-0">
                <div className="bg-white dark:bg-slate-900 rounded-[40px] shadow-2xl shadow-slate-200/50 dark:shadow-none border border-slate-100 dark:border-slate-800 p-8 md:p-16 relative overflow-hidden print:rounded-none print:shadow-none print:border-none print:p-0">
                    ...
                    <div className="relative z-10 flex flex-col gap-12 md:gap-16">
                        ...
                        <div className="flex flex-col md:flex-row justify-between items-start gap-8">
                            <div className="space-y-6">
                                ...
                                <div className="text-slate-400 font-bold text-xs uppercase tracking-widest flex flex-col gap-1">
                            ...
                            <div className="text-left md:text-right space-y-2">
                                ...
                                <div className="flex flex-col md:items-end gap-1">
                                    ...
                                    <span className="text-slate-400 font-bold text-xs uppercase tracking-widest">Diterbitkan: {formattedDate}</span>
                                </div>
                            </div>
                        </div>

                        {/* Details Section */}
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-24 border-t border-b border-slate-100 dark:border-slate-800 py-12">
                            <div>
                                <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mb-4">Ditagih Kepada</h3>
                                <div className="space-y-1">
                                    <p className="text-xl font-black text-slate-900 dark:text-white">{transaction.user?.name}</p>
                                ...
                                <table className="w-full text-left">
                                    ...
                                    <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                                        <tr>
                                            <td className="py-8">
                                                <div className="space-y-1">
                                                    <p className="text-lg font-black text-slate-900 dark:text-white uppercase tracking-tight">Paket {packageName}</p>
                                                    <p className="text-slate-500 dark:text-slate-400 text-xs font-medium">Akses penuh ke fitur premium dan optimasi AI.</p>
                                                </div>
                                            </td>
                                ...
                                <div className="w-full md:w-80 space-y-4">
                                    ...
                                    {discount > 0 && (
                                        <div className="flex justify-between items-center text-sm font-bold text-green-500">
                                            <span className="uppercase tracking-widest text-[10px]">Diskon</span>
                                            <span>-{new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', maximumFractionDigits: 0 }).format(discount)}</span>
                                        </div>
                                    )}
                                    <div className="flex justify-between items-center text-sm font-bold text-slate-500">
                                        <span className="uppercase tracking-widest text-[10px]">Pajak (0%)</span>
                                        <span>Rp0</span>
                                    </div>
                                    <div className="pt-4 border-t-2 border-slate-900 dark:border-white flex justify-between items-center">
                                        <span className="font-black text-xs uppercase tracking-[0.2em] text-slate-900 dark:text-white">Total</span>
